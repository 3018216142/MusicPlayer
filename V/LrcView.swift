...
class LrcView: UITableViewController {
    
    lazy var MidLrcLabel = UILabel()
    
    var MidLrcLabelText : String? = ""
    ...
    var currentCell : LrcTableCell?
    
    var lrcOldCell : LrcTableCell?
    ...
    var Lrc_Index : Int = 0 {
        ...
        didSet{
            ...
            currentCell = cell as! LrcTableCell?
            // 旧的indexPath
            let oldIndexpath = NSIndexPath(row: oldValue, section: 0)
            let oldCell = self.tableView.cellForRow(at: oldIndexpath as IndexPath) as? LrcTableCell
            old_Index = oldValue
            lrcOldCell = oldCell
            cell?.addAnimation(animationType: .scaleAlways)
            oldCell?.addAnimation(animationType: .scaleNormal)
            
            oldCell?.lrcCellTextLabel?.progress = 0
        }
...
extension LrcView {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = LrcTableCell.CellForRowWithTableView(tableView: tableView)
        ...
        if indexPath.row == Lrc_Index {
            ...
            cell.addAnimation(animationType: .scaleAlways)
            cell.lrcCellTextLabel?.progress = lrcProgress
        }else {
            cell.addAnimation(animationType: .scaleNormal)
            cell.lrcCellTextLabel?.progress = 0
        }
...
extension LrcView{
    ...
    fileprivate func autoUpdateLrc()  {
        ...
        if cell != nil {
            currentCell = cell as? LrcTableCell
            currentCell?.lrcCellTextLabel?.progress = lrcProgress
        }
